...
import { Plus, Trash2, TrendingUp, TrendingDown, Wallet, X, PiggyBank } from "lucide-react";
import { subscribeToBudgetData, saveBudgetData } from "./firebase.js";

const CATEGORY_NAME_PRESETS = ["Courses", "Logement", "Transport", "Loisirs", "Santé", "Autre"];

const ACCOUNTS = [
  { id: "melo", name: "Mélo", color: "#B0637A" },
  { id: "theo", name: "Théo", color: "#5C7B8B" },
  { id: "commun", name: "Commun", color: "#C9A857" },
];

function getAccount(id) {
  return ACCOUNTS.find((a) => a.id === id) || ACCOUNTS[2];
}
...
export default function BudgetApp() {
  ...
  const [recurring, setRecurring] = useState([]); // {id, name, amount, type: 'income'|'expense'}
  const [accountBudgets, setAccountBudgets] = useState({}); // { melo: 500, theo: 500, commun: 800 }
  ...
  useEffect(() => {
    const unsubscribe = subscribeToBudgetData((data, error) => {
      if (error) {
        setSaveError(true);
        setLoaded(true);
        return;
      ...
      if (data) {
        ...
        // First run ever: seed with example categories
  ...
  useEffect(() => {
    ...
    saveBudgetData({ budgets, transactions, recurring, accountBudgets, monthlyArchive, savings }).catch(
      () => setSaveError(true)
    );
  }, [budgets, transactions, recurring, accountBudgets, monthlyArchive, savings, loaded]);
  ...
  const spentByAccount = useMemo(() => {
    const map = { melo: 0, theo: 0, commun: 0 };
    for (const t of transactions) {
  ...
  function addBudget(name, limit, defaultAccountId) {
    setBudgets((b) => [...b, { id: uid(), name, limit, defaultAccountId }]);
  }
  ...
  function addTransaction(tx) {
    setTransactions((t) => [{ id: uid(), date: new Date().toISOString(), ...tx }, ...t]);
  }
  ...
  function addRecurring(name, amount, accountId) {
    setRecurring((r) => [...r, { id: uid(), name, amount, type: "income", accountId }]);
  }
  ...
  return (
    <div style={styles.app}>
        ...
        <div style={styles.forecastCard}>
          ...
          {recurring.length === 0 ? (
            ...
            <div style={styles.recurringList}>
              {recurring.map((r) => {
                const account = r.accountId ? getAccount(r.accountId) : null;
                return (
      ...
      {showAddTx && (
        <AddTransactionModal
          key={budgets.length + "-" + budgets.map((b) => b.defaultAccountId).join(",")}
          budgets={budgets}
...
function CategoryDetailModal({ category, budgets, transactions, onClose, onTransfer }) {
...
function AddRecurringModal({ onClose, onAdd }) {
  ...
  const [amount, setAmount] = useState("");
  const [accountId, setAccountId] = useState(ACCOUNTS[2].id);
...
function AddTransactionModal({ budgets, onClose, onAdd }) {
  const [type, setType] = useState("expense");
  const [amount, setAmount] = useState("");
  const [categoryId, setCategoryId] = useState(budgets[0]?.id || "");
  const [accountId, setAccountId] = useState(budgets[0]?.defaultAccountId || ACCOUNTS[2].id);
...
function AddBudgetModal({ onClose, onAdd }) {
  ...
  const [defaultAccountId, setDefaultAccountId] = useState(ACCOUNTS[2].id);
